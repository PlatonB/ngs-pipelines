...
```
python путь_к_пайплайну/имя_пайплайна.py [-h] [-s путь] [-r путь] [-t путь]
                                         [--threads-quan [4]] [--species-name вид]
                                         [--reads-type {paired,unpaired}]
                                         ...
```

Все опции и соответствующие русскоязычные подсказки можно вывести так:
```
python путь_к_пайплайну/имя_пайплайна.py --help
```
...
При небольшом количестве сэмплов и нежелании заморачиваться с GNU Parallel создавайте для каждой задачи свою сессию. Чтобы не запутаться, лучше давать сессиям удобочитаемые имена. Допустим, в формате `имя_программы__фио_клиента__тип_секвенирования`.
```
