...
#### Запуск.
Запустить тот или иной пайплайн с помощью среды _IDLE_, установленной штатным менеджером пакетов, к сожалению, не получится. Но командная строка нам [теперь привычна↑](#преодолеваем-страх-командной-строки-linux), поэтому воспользуемся её услугами:
```
...
Перекидывать со своего компа тексты программ и исходные данные можно с помощью обычных файлменеджеров: Nautilus, Dolphin, [Pantheon Files](https://elementary.io/ru/)... В последнем из перечисленных, к примеру, это делается нажатием `Подключить сервер` в боковом меню.
